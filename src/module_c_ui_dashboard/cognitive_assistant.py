...
def select_assistant_question(typed_question, action):
    """Choose a typed question or the prompt associated with an action button."""
    return typed_question or {
...
def render_assistant(controller, vqa, tts) -> None:
    ...
    question = select_assistant_question(typed_question, action)
    if not question:
        return
